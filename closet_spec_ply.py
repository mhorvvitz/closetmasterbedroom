...
T_DECK = 18            # ply is ~3x stiffer than chipboard; 18 replaces 25
T_TRIM = 12            # shaker trim ripped from the same ply
TRIM_W = 50            # shaker frame width
MAT = "plywood_birch"  # VARIANT: birch ply throughout, exposed edges
...
L_BOX_H, L_BOX_D, BOX_T = 180, 320, 12
...
for tag, x0, dirn, front_x in (("L", 0, +1, TOWER_D),
    ...
    for y0 in DRAWER_Y:
        ...
        c.add(f"{tag}_BoxFB", x=bx + L_BOX_D - BOX_T, y=by, z=bz + BOX_T,
              sx=BOX_T, sy=L_BOX_H, sz=L_BOX_W - 2 * BOX_T, material=PLY, grain="none")
        c.add(f"{tag}_BoxBottom", x=bx + BOX_T, y=by, z=bz + BOX_T,
              sx=L_BOX_D - 2 * BOX_T, sy=4, sz=L_BOX_W - 2 * BOX_T,
              material="hardboard", grain="none")
...
rev = "PLY variant — birch ply + screws"
style = "frameless_kd"
...
banding = {}   # VARIANT: exposed ply edges, no banding



_CAM, _DOW, _SCR = "confirmat", "confirmat", "confirmat"   # VARIANT: all screws
joint_overrides = {}
for tag in ("L", "R"):
